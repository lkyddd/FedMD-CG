...
class ServerManager(BaseServer):
    ...
    def next_step(self):
        ...
        for client_id in self.selected_clients:
            self._ct_.get_node('client', client_id) \
                .fed_client_train_step(step=self.step, global_params=self.global_params) ##当step==0时
    ...
    def fed_finish_client_train_step(self,
        ...
        if self.comm_load[client_id] == 0:
            ...
            self.comm_load[client_id] = (model_size(client_model_params) - model_size(client_classifier_params)) / 1024 / 1024  ##单位为MB
...
class ClientManager(BaseServer):
    ...
    def classifier_to_global_model(self, ):
        classifier_params = self.trainer.classifier.state_dict()
        if self.data_set in ['FMNIST', 'EMNIST', 'CIFAR-10']:
            model_name = self.trainer.model.named_parameter_layers
        classifier_name = list(classifier_params.keys())
        i, j = 0, 0
        for name, param in self.trainer.model.named_parameters():
            if self.data_set in ['FMNIST', 'EMNIST', 'CIFAR-10']:
                if 'fc' in model_name[i]:
                    param.data = classifier_params[classifier_name[j]].clone().detach()
                    j += 1
            else:
                if 'fc' in name:
                    param.data = classifier_params[classifier_name[j]].clone().detach()
    ...
    def fed_client_train_step(self, step, global_params=None):
        self.step = step
        ...
        if self.step > 0:
            self.classifier_to_global_model()  ##将global classifier的参数替换model中对应的层
