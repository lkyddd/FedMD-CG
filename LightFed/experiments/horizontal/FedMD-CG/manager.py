...
class ServerManager(BaseServer):
    ...
    def __init__(self, ct, args):
        ...
        self.client_m_L_CE_aggregator = NumericAvgAgg()
        self.client_m_L_CE_G_aggregator = NumericAvgAgg()
        self.client_m_L_KL_F_aggregator = NumericAvgAgg()
        self.client_m_L_MSE_F_aggregator = NumericAvgAgg()

        self.client_g_L_CE_G_aggregator = NumericAvgAgg()
        self.client_g_L_KL_aggregator = NumericAvgAgg()
        self.client_g_L_MSE_G_aggregator = NumericAvgAgg()
        self.client_g_L_DIV_aggregator = NumericAvgAgg()
    ...
    def init_optimizer(self):
        ...
        # self.gen_gc_optimizer = torch.optim.SGD(
        #     [{'params': self.global_generator_model.parameters(), 'lr': self.lr_gen_gc},
        ...
        for client_id in self.selected_clients:
            self._ct_.get_node('client', client_id) \
                .fed_client_train_step(step=self.step, label_num=self.label_num, latent_layer_idx=self.latent_layer_idx,
                                       global_params=None, global_classifier_params = self.global_classifier_params, global_generator_params=self.global_generator_params) ##当step==0时
    ...
    def proposed_server_aggregation(self, ):
        ...
        for i in range(self.I_gen_gc):
            ...
            for id_, user_idx in enumerate(self.selected_clients):
                ...
                cache_gen_cc_result = self.cache_classifier(cache_gen_result['output'], logit=True)
                cache_gen_cc_softmax_temp = F.softmax(cache_gen_cc_result['logit'] / self.temp_s, dim=1).clone().detach()
        ...
        info_gen = {'comm. round': self.step, 'client_id': 'server',
                    'L_KL': round(L_KL_1, 4), 'L_KL': round(L_KL_2, 4), 'L_KL': round(L_KL_3, 4), 'train_time': round(train_time, 4)}
        # print(info)
    ...
    def fed_finish_client_train_step(self,
        ...
        self.client_m_L_CE_aggregator.put(eval_info['m_L_CE'], weight)
        self.client_m_L_CE_G_aggregator.put(eval_info['m_L_CE_G'], weight)
        self.client_m_L_KL_F_aggregator.put(eval_info['m_L_KL_F'], weight)
        self.client_m_L_MSE_F_aggregator.put(eval_info['m_L_MSE_F'], weight)

        self.client_g_L_CE_G_aggregator.put(eval_info['g_L_CE_G'], weight)
        self.client_g_L_KL_aggregator.put(eval_info['g_L_KL'], weight)
        self.client_g_L_MSE_G_aggregator.put(eval_info['g_L_MSE_G'], weight)
        self.client_g_L_DIV_aggregator.put(eval_info['g_L_DIV'], weight)
        ...
        if not self.unfinished_client_num:
            ...
            m_L_CE = self.client_m_L_CE_aggregator.get_and_clear()
            m_L_CE_G = self.client_m_L_CE_G_aggregator.get_and_clear()
            m_L_KL_F = self.client_m_L_KL_F_aggregator.get_and_clear()
            m_L_MSE_F = self.client_m_L_MSE_F_aggregator.get_and_clear()

            g_L_CE_G = self.client_g_L_CE_G_aggregator.get_and_clear()
            g_L_KL = self.client_g_L_KL_aggregator.get_and_clear()
            g_L_MSE_G = self.client_g_L_MSE_G_aggregator.get_and_clear()
            g_L_DIV = self.client_g_L_DIV_aggregator.get_and_clear()

            print('comm. round: {}, client_test_acc: {:.4f}'.format(self.step, client_test_acc_avg))

            print('comm. round: {}, client_m_L_CE: {:.4f}, client_m_L_CE_G: {:.4f}, client_m_L_KL_F: {:.4f}, client_m_L_MSE_F: {:.4f}'\
                  .format(self.step, m_L_CE, m_L_CE_G, m_L_KL_F, m_L_MSE_F))

            print('comm. round: {}, client_g_L_CE_G: {:.4f}, client_g_L_KL: {:.4f}, client_g_L_MSE_G: {:.4f}, client_g_L_DIV: {:.4f}'\
                  .format(self.step, g_L_CE_G, g_L_KL, g_L_MSE_G, g_L_DIV))
            ...
            if self.step == self.comm_round - 1:
                ...
                self.finall_models = [self.model, self.global_classifier, self.global_generator_model, ]
...
class ClientManager(BaseServer):
    ...
    def fed_client_train_step(self, step, label_num, latent_layer_idx, global_params=None, global_classifier_params=None, global_generator_params=None):
        self.step = step
        ...
        if step > 0:
            ...
            if step == 1:
                self.trainer.EMA_generator_model.load_state_dict(global_generator_params, strict=True)
            elif step > 1:
                EMA_generator_params = get_cpu_param(self.trainer.EMA_generator_model.state_dict())
                EMA_generator_params = formula(lambda EMA_gen_param, gen_param: self.alpha * EMA_gen_param + (1 - self.alpha) * gen_param,
                    EMA_generator_params, global_generator_params)
                self.trainer.EMA_generator_model.load_state_dict(EMA_generator_params, strict=True)

        # self.trainer.generator_model = torch.load(f"{path}/model_save/{self.generator_model_type}_{self.data_set}.pth")
        # self.trainer.generator_model.load_state_dict(global_generator_params, strict=True)
